...
import time, datetime, logging, requests, json, xmltodict, sys
...
class SecuritasAPIClient:
    ...
    def _api_requests(self, request, **more):
        ...
        if "ID" not in more:
            self._set_id()
        resp = requests.get(self.BASE_URL, params={**self._params, **more}, timeout=15)
        # _LOGGER.debug("response code %s"%resp.status_code)
        # _LOGGER.debug("response body %s"%resp.text)
        ...
        if resp.status_code == 200:
            ...
            if self._is_ok(res):
                ...
                if self._is_ok(res):
                    ...
                    self._params.update(
                        {"numinst": inst["NUMINST"], "panel": inst["PANEL"]}
                    )
...
if __name__ == "__main__":
    ...
    if "PET" in out and "BLOQ" in out["PET"]:
        del out["PET"]["BLOQ"]
